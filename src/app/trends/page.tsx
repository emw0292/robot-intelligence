...
export default async function TrendsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const { data: filtered, error } = await getPublishedTrends({ q, country, category, importance });

  return (
    <div className="shell page-shell">
      <PageIntro eyebrow="INTELLIGENCE FEED" title="최신 동향" description="국내외 로봇·AI 정책, 산업, 기술, R&D, 표준·인증 변화를 통합 탐색합니다." stats={[{ label: "공개 데이터", value: error ? "-" : `${filtered.length}건` }, { label: "데이터 소스", value: "Supabase" }]} />
      ...
      {error ? <div className="empty-state data-error-state" role="alert"><strong>데이터를 불러오지 못했습니다.</strong><p>{error}</p></div> : filtered.length ? <div className="trend-grid">{filtered.map((trend) => <TrendCard key={trend.id} trend={trend} />)}</div> : <div className="empty-state"><strong>조건에 맞는 동향이 없습니다.</strong><p>검색어 또는 필터를 변경해 다시 확인해 주세요.</p></div>}
    </div>
  );
}
